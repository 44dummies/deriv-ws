...
const AdminControlPanel: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'overview' | 'sessions' | 'accounts' | 'logs'>('overview');
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.tabs}>
        {(['overview', 'sessions', 'accounts', 'logs'] as const).map((tab) => (
          <button
            key={tab}
            style={{
              ...styles.tab,
...
const SessionRow: React.FC<{ session: TradingSession }> = ({ session }) => {
...
const AccountRow: React.FC<{ account: TradingAccount }> = ({ account }) => {
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'active': return '#22c55e';
...
const CreateSessionModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      sessionManager.createSession(formData as CreateSessionRequest, 'admin');
      onClose();
    } catch (error: any) {
      alert(error.message);
    }
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={styles.formGroup}>
            ...
            <select
              ...
              onChange={(e) => setFormData({ ...formData, stakingMode: e.target.value as any })}
            >
