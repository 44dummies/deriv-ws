...
import {
  ...
  CheckCircle, XCircle, AlertCircle, Bell, Play, Pause,
...
const UserTradingDashboard = ({ user }) => {
  ...
  const { 
    ...
    lastTick, 
    isConnected 
  } = useTickStream(activeSession?.volatility_index);
  ...
  useEffect(() => {
    const loadTrades = async () => {
      ...
      try {
        const data = await tradingApi.getTrades({ sessionId: selectedSession.id });
        setTrades(data);
      } catch (error) {
  ...
  const handleAcceptInvitation = async (invitationId) => {
    try {
      await tradingApi.acceptInvitation(invitationId);
  ...
  const handleDeclineInvitation = async (invitationId) => {
    try {
      await tradingApi.declineInvitation(invitationId);
      setPendingInvitations(prev => prev.filter(i => i.id !== invitationId));
    } catch (error) {
