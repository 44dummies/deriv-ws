...
interface ChatroomAssignment {
  tier_chatrooms: TierChatroom;
}
...
const fileStorage = {
   ...
   */
  uploadFile: async (file: File, context = 'chatroom'): Promise<any> => {
   ...
   */
  uploadVoice: async (file: File, duration: number | null = null): Promise<any> => {
    const formData = new FormData();
   ...
   */
  saveFileMetadata: (fileId: string, metadata: any): void => {
 ...
 */
async function getTierChatrooms(): Promise<{ success: boolean; chatrooms: TierChatroom[] }> {
  try {
    const response = await apiClient.get<any>(`${API_BASE}/tier-chatrooms`);
    return response;
  } catch (error) {
 ...
 */
async function getMyTierChatroom(): Promise<{ success: boolean; assignment: ChatroomAssignment | null }> {
  try {
    const response = await apiClient.get<any>(`${API_BASE}/my-tier-chatroom`);
    return response;
  } catch (error) {
 ...
 */
async function assignToTierChatroom(winRate: number, totalTrades: number): Promise<any> {
  try {
    const response = await apiClient.post<any>(`${API_BASE}/assign-tier`, {
      winRate,
      totalTrades
    });
    return response;
 ...
 */
async function getChatroomMembers(chatroomId: string, limit: number = 50): Promise<any> {
  try {
    const response = await apiClient.get<any>(`${API_BASE}/tier-chatroom/${chatroomId}/members`, {
      params: { limit }
    });
    return response;
  } catch (error) {
 ...
 */
async function getChatroomMessages(chatroomId: string, limit: number = 50, before: string | null = null): Promise<any> {
  try {
    const params: any = { limit };
    if (before) params.before = before;

    const response = await apiClient.get<any>(`${API_BASE}/tier-chatroom/${chatroomId}/messages`, {
      params
    });
    return response;
  } catch (error) {
 ...
 * Send a text message
 */
async function sendMessage(chatroomId: string, text: string, replyToId: string | null = null): Promise<any> {
  try {
    const response = await apiClient.post<any>(`${API_BASE}/tier-chatroom/${chatroomId}/message`, {
      text,
      type: 'text',
      replyToId
    });
 ...
 */
async function sendFileMessage(chatroomId: string, file: File, caption: string | null = null): Promise<any> {
  try {
 ...
 */
async function addReaction(messageId: string, emoji: string): Promise<any> {
  try {
    const response = await apiClient.post<any>(`${API_BASE}/tier-message/${messageId}/reaction`, {
      emoji
    });
    return response;
  } catch (error: any) {
 ...
 * Delete a message
 */
async function deleteMessage(messageId: string): Promise<any> {
  try {
    const response = await apiClient.delete<any>(`${API_BASE}/tier-message/${messageId}`);
    return response;
  } catch (error: any) {
