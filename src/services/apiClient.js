    ...
    try {
      ...
      if (response.status === 401 && this.refreshToken) {
        ...
        if (refreshed) {
          ...
          // Only force logout if we REALLY failed refresh
          // But if the URL is an admin check, maybe we shouldn't kill the whole session?
          // For now, let's just be sure we clear tokens only on hard failure
          this.clearTokens();
          if (this.onAuthError) {
            this.onAuthError();
          }
