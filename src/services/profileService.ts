...
class ProfileService {
  ...
  async updateProfile(updates) {
    try {
      const response = await apiClient.put<ProfileUpdateResponse>('/users/me', updates);
  ...
  async updateUsername(username) {
    try {
      const response = await apiClient.put<UsernameUpdateResponse>('/users/me/username', { username });
  ...
  async uploadAvatar(file) {
    try {
      ...
      const response = await apiClient.uploadFile<AvatarUploadResponse>('/users/me/avatar', formData);
  ...
  async removeAvatar() {
    try {
      const response = await apiClient.delete<AvatarDeleteResponse>('/users/me/avatar');
  ...
  async getUserByDerivId(derivId) {
    ...
    try {
      const response = await apiClient.get<any>(`/users/${derivId}`);
      if (response) {
