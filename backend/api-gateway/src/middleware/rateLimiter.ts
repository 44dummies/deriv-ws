...
// Stricter limits for auth login/callback endpoints (not session check)
const AUTH_CONFIG: RateLimitConfig = {
    windowMs: 15 * 60 * 1000,      // 15 minutes
    maxRequests: 30,               // 30 attempts per 15 minutes (increased for OAuth flow)
    message: 'Too many authentication attempts, please try again later.',
    keyPrefix: 'tradermind:ratelimit:auth:'
};

// Session check rate limit (called frequently for auth state)
const SESSION_CHECK_CONFIG: RateLimitConfig = {
    windowMs: 60 * 1000,           // 1 minute
    maxRequests: 60,               // 60 session checks per minute (generous for SPA)
    message: 'Too many session checks, please slow down.',
    keyPrefix: 'tradermind:ratelimit:session:'
};
...
export const sessionCheckRateLimiter = createRateLimiter(SESSION_CHECK_CONFIG);
