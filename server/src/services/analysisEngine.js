 ...
 * Implements 8 trading strategies for digit analysis
 * Adapted from frontend strategyEngine.js for backend use
 */

// Contract types
const CONTRACT_TYPE = {
    DIGITMATCH: 'DIGITEVEN',
    DIGITDIFF: 'DIGITDIFF',
    DIGITEVEN: 'DIGITEVEN',
    DIGITODD: 'DIGITODD',
    DIGITOVER: 'DIGITOVER',
 ...
 */
function analyzeForSignal(ticks, preferredStrategy = null, minConfidence = 50) {
    ...
    if (preferredStrategy && preferredStrategy !== 'ALL') {
        ...
        if (signal.confidence < minConfidence) return { shouldTrade: false, reason: 'Low confidence', signal };
    ...
    if (aggregated.confidence < minConfidence) return { shouldTrade: false, reason: 'Low aggregated confidence', signal: aggregated };
